...
/*                                                 #+#   #+    #+    #+#      */
/*   Created: 2019/02/08 14:58:39 by gmonacho     #+#   ##    ##    #+#       */
/*   Updated: 2019/02/08 16:03:12 by gmonacho    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static void		disp_ver(t_map map, t_window *window)
{
	...
	while (j < map.size.j)
	{
		...
		while (i >= 0)
		{
			p1 = create_vpoint(j - ft_round(map.cursor.x),
					i - ft_round(map.cursor.y), map.tab[i][j], map);
			if (i - 1 >= 0)
			{
